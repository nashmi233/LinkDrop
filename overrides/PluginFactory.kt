 ...
 *
 * LinkDrop V1 modification: simple mode.
 */
...
object PluginFactory {
    annotation class LoadablePlugin

    private val simpleModePluginClasses = setOf(
        "SharePlugin",
        "ClipboardPlugin",
        "PingPlugin"
    )
    ...
    fun initPluginInfo(context: Context) {
        try {
            pluginInfo = com.albertvaka.classindexksp.LoadablePlugin
                ...
                .filter { plugin -> plugin.javaClass.simpleName in simpleModePluginClasses }
                .associate { Pair(it.pluginKey, PluginInfo(it)) }
        } catch (e: Exception) {
            throw RuntimeException(e)
        }
        Log.i("PluginFactory", "LinkDrop simple mode loaded " + pluginInfo.size + " plugins")
    }
    ...
    val incomingCapabilities: Set<String>
        get() = pluginInfo.values.flatMap { it.supportedPacketTypes }.toSet()

    val outgoingCapabilities: Set<String>
        get() = pluginInfo.values.flatMap { it.outgoingPacketTypes }.toSet()
    ...
    fun pluginsForCapabilities(incoming: Set<String>, outgoing: Set<String>): Set<String> {
        ...
        unused.forEach { Log.d("PluginFactory", "Won't load ${it.key}: unmatched capabilities") }
    ...
    class PluginInfo private constructor(
        ...
        internal constructor(p: Plugin) : this(
            p.displayName,
            p.description,
            p.isEnabledByDefault,
            p.hasSettings(),
            p.listensToUnpairedDevices(),
            p.supportedPacketTypes,
            p.outgoingPacketTypes,
            p.javaClass
        )
